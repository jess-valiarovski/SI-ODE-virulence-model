...
m_t = seq(0,1,0.01)
m_r = seq(0,1,0.01)
alpha = seq(0.1,1,0.1) #virulence
parameter_sweep <- expand.grid(m_t = seq(0,1,0.01),
                          m_r = seq(0,1,0.01),
                          beta = 0.4,
                          gamma = 0.01,
                          miu_s = 0.1,
...
#generate figure of simulation output (needs to be fixed up)
sp2 <- ggplot(parameters, aes(x=m_t,y=m_r, color = avg_class)) + geom_point() 
sp2 +scale_color_gradient(low="blue", high="red")
mid<-mean(parameters$avg_class)
sp2+scale_color_gradient2(midpoint=mid, low="blue", mid="white",
                          high="red", space ="Lab" )
sp3 <- ggplot(parameters, aes(x=m_t,y=m_r, fill = avg_class)) + geom_tile() 
